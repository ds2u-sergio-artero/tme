...
// Anchor on the refspec position so a branch merely CONTAINING the word
// (feat/main-menu, fix/master-list) is not treated as targeting the trunk.
const pushTargetsMain =
  isPush && /\s(?:origin\s+)?(?:main|master)(?::\S+)?(?:\s|$)/.test(command);
